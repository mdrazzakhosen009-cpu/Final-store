# Made by Nexora WEB — Premium Full-Stack Clothing Store
...
## Included
- Made by Nexora WEB logo and dark navy/gold premium theme
...
## Nexora / AI setup
- Storefront WhatsApp: 01753519603
- Admin route: `/admin` (not linked from storefront)
- Optional real ChatGPT assistant: set `OPENAI_API_KEY` on the server. Optional `OPENAI_MODEL` defaults to `gpt-5.6-luna`. Without a key, the site falls back to the built-in shopping concierge so product search and shopping actions still work.
- Footer credit: Made by Nexora WEB
